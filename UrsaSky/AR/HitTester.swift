...
enum HitTester {
    ...
    /// Uses directions computed at the last sky rebuild — no per-tap Meeus.
    static func nearestCached(
        tap: CGPoint,
        in view: SCNView,
        targets: [(star: Star, direction: SIMD3<Double>)],
        maxDegrees: Double = 2.2
    ) -> Star? {
        let near = SCNVector3(Float(tap.x), Float(tap.y), 0)
        let far = SCNVector3(Float(tap.x), Float(tap.y), 1)
        ...
        for target in targets {
            let v = simd_normalize(target.direction)
            let ang = Angle.rad(acos(max(-1, min(1, simd_dot(v, dir)))))
            let limit = maxDegrees + max(0, (3.5 - target.star.mag) * 0.25)
            if ang < bestAng && ang < limit {
                bestAng = ang
                best = target.star
            }
